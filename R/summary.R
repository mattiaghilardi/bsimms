...
print.bsimms_fit <- function(x, ...) {
  ...
  cat(" sources (K):     ", spec$K, " (", paste(spec$source_names, collapse = ", "), ")\n", sep = "")
  cat(" isotopes (J):    ", spec$J, " (", paste(spec$isotope_names, collapse = ", "), ")\n", sep = "")
  cat(" mixtures (N):    ", spec$N, "\n", sep = "")
  cat(" error structure: ", spec$error_structure, "\n", sep = "")
  cat(" source data:     ", spec$source$mode, "\n", sep = "")
  cat(" tdf data:        ", spec$tdf$mode, "\n", sep = "")
  if (spec$has_conc_dep) cat(" concentration dependence: yes\n")
  if (length(spec$re_terms) > 0) {
    cat(" group-level terms:\n")
    for (re in spec$re_terms) {
      cat("   (", paste(re$term_names, collapse = " + "), " | ", re$group, "), ",
          length(re$group_levels), " levels\n", sep = "")
    }
  }
...
#' @export
summary.bsimms_fit <- function(object, robust = FALSE, probs = c(0.025, 0.975), ...) {
  spec <- object$spec
  measures <- c(
    if (robust) list("median", "mad") else list("mean", "sd"),
    list(~ posterior::quantile2(.x, probs = probs), "rhat", "ess_bulk", "ess_tail")
  )
  ...
  re_summary <- NULL
  if (length(spec$re_terms) > 0) {
    re_summary <- do.call(rbind, lapply(spec$re_terms, function(re) {
      d <- summarise(bsimms_draws(object, variable = paste0("sd_re_", re$label)))
      d$group <- re$group
  ...
  if (spec$error_structure %in% c("residual_only", "process_residual")) {
    err_var <- if (spec$error_structure == "residual_only") "sigma" else "resid_prop"
  ...
  out <- list(
    call = object$call, spec = spec, robust = robust,
    fixed = fixed, group = re_summary, error = err_summary,
...
print.summary.bsimms_fit <- function(x, digits = 3, ...) {
  ...
  non_quant_cols <- c("variable", "mean", "sd", "median", "mad", diag_cols,
                       "source", "fixed_effect", "ilr_dim", "isotope", "group", "term")
  quant_cols <- function(d) setdiff(names(d), non_quant_cols)
  round_table <- function(d, id_cols) {
    for (col in setdiff(names(d), id_cols)) {
      d[[col]] <- round(d[[col]], if (col %in% c("ess_bulk", "ess_tail")) 0 else digits)
    }
    d
  }

  cat("Population-average source proportions:\n")
  pp <- x$population_proportions[, c("source", est_cols, quant_cols(x$population_proportions), diag_cols)]
  pp <- round_table(pp, "source")
  print(pp, row.names = FALSE)

  if (!is.null(x$fixed)) {
    cat("\nFixed effects (ILR scale, deviations from the global proportions):\n")
    fe <- x$fixed[, c("fixed_effect", "ilr_dim", est_cols, quant_cols(x$fixed), diag_cols)]
  ...
  if (!is.null(x$group)) {
    ...
    gg <- x$group[, c("group", "term", "ilr_dim", est_cols, quant_cols(x$group), diag_cols)]
