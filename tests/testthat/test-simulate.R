...
test_that("raw vs summarised source/tdf data have the expected shape", {
  ...
  sim_summary <- simulate_bsimms_data(
    n_mixture_obs = 10,
    n_sources = 2,
    source_means_sds = TRUE,
    seed = 1
  ...
  expect_named(
    sim_summary$source_data,
    c("Source", "isotope1_mean", "isotope1_sd", "isotope2_mean", "isotope2_sd")
  )
})
...
test_that("conc_dep adds <isotope>_conc columns for raw and summarised source data", {
  ...
  expect_true(all(
    c("isotope1_conc", "isotope2_conc") %in% names(sim_raw$source_data)
  ))
  expect_true(all(
    sim_raw$source_data$isotope1_conc > 0 &
      sim_raw$source_data$isotope1_conc <= 1
  ))
  ...
  expect_true(all(
    c("isotope1_conc", "isotope2_conc") %in% names(sim_summary$source_data)
  ))
})
...
test_that("simulated data validates against make_standata() across configurations", {
  ...
  for (i in seq_len(nrow(configs))) {
    ...
    sim <- simulate_bsimms_data(
      ...
      n_mixture_obs = 30,
      n_groups = list(Region = 3),
      source_means_sds = cfg$source_means_sds,
      tdf_means_sds = cfg$tdf_means_sds,
      error_structure = cfg$error_structure,
      conc_dep = cfg$conc_dep,
      seed = 1
    )
    sdata <- make_standata(
      sim$formula,
