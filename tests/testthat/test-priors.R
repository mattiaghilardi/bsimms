...
test_that("a more specific override wins over a cascaded general one, regardless of order", {
  ...
  expect_identical(
    merged1$prior[merged1$class == "resid_prop"],
    merged2$prior[merged2$class == "resid_prop"]
  )
...
test_that("a resp-only override cascades onto every source, leaving other isotopes alone", {
  ...
  merged <- merge_bsimms_prior(default, user, spec)
  rows <- merged[
    merged$class == "source_mean" & merged$resp == "d13C" & merged$group != "",
  ]
  expect_true(all(rows$prior == "normal(0, 50)"))
  untouched <- default[
    default$class == "source_mean" & default$resp == "d15N",
  ]
  merged_d15n <- merged[
    merged$class == "source_mean" & merged$resp == "d15N" & merged$group != "",
  ]
...
test_that("merge_bsimms_prior errors on an unrecognised coef/resp/group", {
  ...
  expect_snapshot(
    ...
    merge_bsimms_prior(
      default,
      bsimms_prior("normal(0, 1)", class = "b", coef = "SexZZZ"),
      spec_re
    )
  )
  expect_snapshot(
    ...
    merge_bsimms_prior(
      default,
      bsimms_prior("2", class = "p_global", group = "Beavr"),
      spec_re
    )
  )
  expect_snapshot(
    ...
    merge_bsimms_prior(
      default,
      bsimms_prior("student_t(3, 0, 1)", class = "sd", group = "Regionn"),
      spec_re
    )
  )
})
...
test_that("merge_bsimms_prior accepts valid coef/resp/group without error", {
  default <- default_bsimms_prior(spec_re)
  merged <- merge_bsimms_prior(
    default,
    bsimms_prior("normal(0, 1)", class = "b", coef = "SexM"),
    spec_re
  )
  expect_equal(
    merged$prior[merged$class == "b" & merged$coef == "SexM"],
    "normal(0, 1)"
  )
})
...
test_that("select_prior falls back from most to least specific", {
  ...
  expect_equal(
    select_prior(df, "sd", coef = "x", group = "Region"),
    "student_t(3, 0, 3)"
  )
  expect_equal(
    select_prior(df, "sd", coef = "y", group = "Region"),
    "student_t(3, 0, 2)"
  )
  expect_equal(
    select_prior(df, "sd", coef = "y", group = "Other"),
    "student_t(3, 0, 1)"
  )
})
...
test_that("select_prior falls back across coef, resp and group jointly", {
  ...
  expect_equal(
    select_prior(df, "source_sd", resp = "d13C", group = "Beaver"),
    "student_t(3, 0, 4)"
  )
  expect_equal(
    select_prior(df, "source_sd", resp = "d13C", group = "Deer"),
    "student_t(3, 0, 3)"
  )
  expect_equal(
    select_prior(df, "source_sd", resp = "d15N", group = "Beaver"),
    "student_t(3, 0, 2)"
  )
  expect_equal(
    select_prior(df, "source_sd", resp = "d15N", group = "Deer"),
    "student_t(3, 0, 1)"
  )
})
...
test_that("bsimms_get_prior returns default priors for a simple model", {
  ...
  expect_s3_class(p, "bsimms_prior")
  expect_true(all(
    c("b", "p_global", "resid_prop", "source_mean", "source_sd") %in% p$class
  ))
