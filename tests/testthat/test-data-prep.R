...
test_that("infer_source_names errors on a non-string source_col", {
  ...
  expect_snapshot(
    error = TRUE,
    infer_source_names(source_data = sources, tdf_data = NULL, source_col = c("Source", "Other"))
  )
  expect_snapshot(
    error = TRUE,
...
test_that("infer_source_names errors on missing source_col", {
  ...
  expect_snapshot(
    error = TRUE,
    infer_source_names(source_data = data.frame(Source = "A"), tdf_data = data.frame(x = 1))
  )
})
...
test_that("infer_source_names errors on source/tdf name mismatch", {
  ...
  expect_snapshot(error = TRUE, infer_source_names(source_data = sources, tdf_data = tdf))
})
...
test_that("prep_mixture_isotopes builds a numeric matrix in isotope_names order", {
  ...
  y <- prep_mixture_isotopes(mixture_data = d, isotope_names = c("d13C", "d15N"))
  expect_equal(y, matrix(c(3, 4, 1, 2), nrow = 2, dimnames = list(NULL, c("d13C", "d15N"))))
})

test_that("prep_mixture_isotopes errors on missing isotope columns", {
  expect_snapshot(
    error = TRUE,
    prep_mixture_isotopes(mixture_data = data.frame(d13C = 1), isotope_names = c("d13C", "d15N"))
  )
})
...
test_that("prep_iso_table (raw) builds source_idx and Y correctly", {
  d <- data.frame(Source = c("Deer", "Beaver", "Deer"), d13C = c(1, 2, 3), d15N = c(4, 5, 6))
  out <- prep_iso_table(
    data = d, isotope_names = c("d13C", "d15N"), source_names = c("Beaver", "Deer"),
    means_sds = FALSE
  )
...
test_that("prep_iso_table errors on an invalid label", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_iso_table(
      data = d, isotope_names = "d13C", source_names = "Beaver",
...
test_that("prep_iso_table (raw) errors on missing source_col", {
  expect_snapshot(
    error = TRUE,
    prep_iso_table(
      data = data.frame(x = 1), isotope_names = "d13C", source_names = "Beaver",
      means_sds = FALSE
    )
  )
})
...
test_that("prep_iso_table (raw) errors on unknown source", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_iso_table(
      data = d, isotope_names = "d13C", source_names = c("Beaver", "Deer"),
      means_sds = FALSE
    )
  )
})
...
test_that("prep_iso_table (raw) errors on missing isotope columns", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_iso_table(
      data = d, isotope_names = c("d13C", "d15N"), source_names = "Beaver",
      means_sds = FALSE
    )
  )
})
...
test_that("prep_iso_table (raw) errors on missing values", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_iso_table(data = d, isotope_names = "d13C", source_names = "Beaver", means_sds = FALSE)
  )
})
...
test_that("prep_iso_table (summary) reorders rows to match source_names", {
  d <- data.frame(
    ...
    d13C_mean = c(-28, -26), d13C_sd = c(0.8, 1.0),
    d15N_mean = c(2, 3), d15N_sd = c(0.5, 0.6)
  )
  out <- prep_iso_table(
    data = d, isotope_names = c("d13C", "d15N"), source_names = c("Beaver", "Deer"),
    means_sds = TRUE
...
test_that("prep_iso_table (summary) errors on missing mean/sd columns", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_iso_table(data = d, isotope_names = "d13C", source_names = "Beaver", means_sds = TRUE)
  )
})

test_that("prep_iso_table (summary) errors on duplicated source rows", {
  d <- data.frame(Source = c("Beaver", "Beaver"), d13C_mean = c(-26, -27), d13C_sd = c(1, 1))
  expect_snapshot(
    error = TRUE,
    prep_iso_table(data = d, isotope_names = "d13C", source_names = "Beaver", means_sds = TRUE)
  )
...
test_that("prep_conc_dep returns NULL when conc_dep is FALSE", {
  expect_null(prep_conc_dep(
    source_data = data.frame(Source = "Beaver"), isotope_names = "d13C", source_names = "Beaver",
    source_means_sds = FALSE, conc_dep = FALSE
  ))
})
...
test_that("prep_conc_dep errors when conc_dep is not TRUE/FALSE", {
  expect_snapshot(
    ...
    prep_conc_dep(
      source_data = data.frame(Source = "Beaver"), isotope_names = "d13C", source_names = "Beaver",
      source_means_sds = FALSE, conc_dep = "yes"
    )
  )
})

test_that("prep_conc_dep (raw) averages per-sample concentrations by source", {
  d <- data.frame(Source = c("Beaver", "Beaver", "Deer"), d13C_conc = c(0.3, 0.5, 0.4))
  m <- prep_conc_dep(
    source_data = d, isotope_names = "d13C", source_names = c("Beaver", "Deer"),
...
test_that("prep_conc_dep (raw) handles multiple isotopes with the right shape", {
  d <- data.frame(
    ...
    d13C_conc = c(0.3, 0.5, 0.4), d15N_conc = c(0.05, 0.05, 0.04)
  )
  m <- prep_conc_dep(
    source_data = d, isotope_names = c("d13C", "d15N"), source_names = c("Beaver", "Deer"),
    source_means_sds = FALSE, conc_dep = TRUE
  )
...
test_that("prep_conc_dep errors on missing concentration columns", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_conc_dep(
      source_data = d, isotope_names = "d13C", source_names = "Beaver",
      source_means_sds = FALSE, conc_dep = TRUE
    )
  )
})
...
test_that("prep_conc_dep errors when a source's concentrations sum to more than 1", {
  ...
  expect_snapshot(
    error = TRUE,
    prep_conc_dep(
      source_data = d, isotope_names = c("d13C", "d15N"), source_names = "Beaver",
      source_means_sds = TRUE, conc_dep = TRUE
    )
  )
})
